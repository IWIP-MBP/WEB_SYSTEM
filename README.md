...
*   **员工花名册全局检索（支持中文输入）**：已彻底清理会导致中文输入法（IME）中断和光标丢失的动态实时搜索 JS 脚本，完全恢复浏览器原生的中英文流畅输入体验。输入搜索内容后，按下回车或点击“搜索”按钮即可在表单中提交查询。且在搜索词或任意筛选条件变更时自动重置页码为第 1 页，解决由于高页码导致搜索结果显示为空的问题。
*   **词典补全与“员工状态”标准化**：补全多语言字典与表头映射，中文界面统一标注为 **员工状态**，印尼文界面标注为 **Status Karyawan**，彻底解决原先回显变量名 `staus_staus` 的问题。
*   **按归属公司筛选与导出**：员工花名册筛选区新增“归属公司”下拉菜单，后端与导出 Excel 功能同步支持归属公司筛选。
*   **岗位列按语言动态切换**：切换系统语言为印尼文时，花名册表格自动显示“岗位(印)” (`pos_id_jabatan`)；切回中文界面时自动恢复显示“岗位(中)” (`pos_cn_jabatan`)。
*   **系统登录页介绍在线自定义**：登录界面动态获取系统介绍文本，管理员可在【系统设置 ⚙️】页面中为登录页在线修改与保存中文及印尼文系统介绍。
